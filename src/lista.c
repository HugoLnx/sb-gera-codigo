...
/*****  Código das funções exportadas pelo módulo  *****/
   ...
   unsigned char* LIS_ToArray(int paramsSize, ...)
   {
      va_list pa;
      int i, j;
      int elemsSize = 0;
      int elemsPreenchidos = 0;
      LIS_tppLista *pParams;
      unsigned char *arr;

      va_start(pa, paramsSize);

      pParams = (LIS_tppLista*) malloc(sizeof(LIS_tppLista)*paramsSize);
      for (i = 0; i < paramsSize; i++)
      {
         pParams[i] = va_arg(pa, LIS_tppLista);
         elemsSize += pParams[i]->numElem;
      }

      arr = (unsigned char*) malloc(sizeof(unsigned char)*elemsSize);

      for (i = 0; i < paramsSize; i++)
      {
         int size;
         size = pParams[i]->numElem;
				 LIS_IrInicioLista(pParams[i]);
         for (j = 0; j < size; j++)
         {
            unsigned char valor;
            LIS_ObterValor(pParams[i], &valor);
            arr[elemsPreenchidos++] = valor;
						LIS_AvancarElementoCorrente(pParams[i], 1);
         }
      }
...
*  ****/
   LIS_tpCondRet LIS_ObterValor(LIS_tppLista pLista, unsigned char *pValor)
   {
      if (pLista->pElemCorr == NULL)
      {
        return LIS_CondRetListaVazia;
